/*
 ============================================================================
 Name        : Weather.c
 Description : Contains all functions to read temp. sensors and weather station
 Author      : Palinecko
 Version     : 2.0
...
int readTemperatureSensors(void)
{
    ...
    ctx = modbus_new_rtu("/dev/ttyUSBt", 9600,'N',8,1); // is connected to USB port 1
    if (ctx == NULL)
    {
...
#define SERVER_ID 1
    ...
    ctx = modbus_new_rtu("/dev/ttyUSBw", 19200,'N',8,1);
