...
int main(void) {
	...
	printf("the time taken was %ld \n",diff/10000);
	n = 15 -diff/10000;
	sleep(n); /*pause before next iteration*/
	/*NEW THREAD IS CREATED every now and then to write weather values to the server */
	if(j==6){
		/*write weather data*/
	...
	if(j==15){
			/*write Victron data*/
			iret3 = pthread_create(&thread4,NULL,functionVictronData,NULL);
			if(iret2)
					{
						fprintf(stderr,"Error-pthread_create() return code: %d \n",iret2);
						exit(EXIT_FAILURE);
...
/*READ DATA FROM VICTRON AND SEND THEM TO THE SERVER*/
	uint16_t *victron;
		...
		victron= getData(3,38); //this is repeating down, the data are read in three big chunks
