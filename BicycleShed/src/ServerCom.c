...
int writeWeatherToServer(uint16_t *weather){
	...
	if (curl){
		CURLcode res;
		char link[100];
		// LET BIJ NIEUWE VERSIE VAN SERVERCOM OP SCHALING (1e 5 half-words zijn goed geschaald, volgende 6 delen door 10, laatste delen door 100)
		int vys = sprintf(link,"http://solarpoweredbikes.tudelft.nl/test/php/Pavel.php?w1=%d&w2=%d&w3=%d&w4=%d&w5=%d&w6=%d",*(weather+5)/10,*(weather+6)/10,*(weather+7)/10,*(weather+8)/10,*(weather+9)/100,*(weather+10)/10);///10 is the scalefactor, except for precipation, which is 100.
		puts(link);
...
int writeVictron3ToServer(uint16_t *victron){
	...
	if (curl){
		...
		int vys = sprintf(link,"http://solarpoweredbikes.tudelft.nl/test/php/Victron3.php"
				"?w1=%d&w2=%d&w3=%d&w4=%d&w5=%d&w6=%d&w7=%d",*(victron),*(victron+1),*(victron+2),*(victron+3)
