...
void htmlDisplay(){
	...
	/* registers 2 - 6 (sensor statuses), 34 (temperature), 39 (wind-chilltemperature), 13 (relative humidity),
	   45 (wind speed), 30 (radiation), 17 (relative air pressure), 48 (precipitation), 18 (wind direction) */

	int registersToRead[] = {2, 3, 4, 5, 6, 34, 39, 13, 45, 30, 17, 48, 18};
	int length = sizeof(registersToRead)/sizeof(int);
	double *weather = readWeatherStation(registersToRead, length);

//    int i;
//    for(i=0; i<13; i++){
//        printf("RegisterstoRead[%d] = %f", registersToRead[i], weather[i]);
//    }

	if (weather){ //Check if weather was correctly fetched
        ...
        fprintf(fp,"<td>%f kPa</td>\n",*(weather+9));
        fprintf(fp,"</tr>\n");
        fprintf(fp,"<tr>\n");
        fprintf(fp,"<td>Wind Speed</td>\n");
        fprintf(fp,"<td>%f m/s</td>\n",*(weather+7));
...
void WeatherDisplay(){
	...
	weather = readWeatherStation(registersToRead, length);
