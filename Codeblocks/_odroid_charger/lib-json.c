...
int readChunk(struct libJson* lj) {
  ...
  for(i = 0; i < length; i++)
    lj->bufferAnswer[i] = '\0';
  n = read(lj->sockfd, lj->bufferAnswer, length);
  if(n < 0) {
...
char* findMessage(struct libJson* lj) {
  int i, length = sizeof(lj->bufferAnswer) / sizeof(*lj->bufferAnswer);
  for(i = 0; i < length; i++) {
...
int getContentLength(struct libJson* lj) {
  int i, ret, length = sizeof(lj->bufferAnswer) / sizeof(*lj->bufferAnswer);
  for(i = 0; i < length; i++)
    if(lj->bufferAnswer[i] == '\n') {
...
int readAns(struct libJson* lj) {
  int i, j, length = sizeof(lj->bufferAnswer) / sizeof(*lj->bufferAnswer);
  int size;
  char* tmp;
