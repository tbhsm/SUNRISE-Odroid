...
char* json_encode_arr(double* arr, int size) {
  // initial length of the string, counting the "[]", de "," and the " "
  int length = size*2+1;
  ...
  for(i = 0; i < size; i++) {
    cur = arr[i];
    // The number '0' takes up a character, but it is an end condition for the loop below. So it has to be treated separately.
    if(cur == 0) {
      length++;
      continue;
    }
    // The minus sign. (Not an actual issue for uint16_t var's, but this is more flexible if the type is going to be changed).
    if(cur < 0) length++;
    // Example for this loop:  when cur = 100
    // -> length = 1, cur = 10
    // -> length = 2, cur = 1
    // -> length = 3, cur = 0
    // stop the loop
