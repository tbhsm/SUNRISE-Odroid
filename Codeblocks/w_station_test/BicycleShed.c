...
void *functionData(){
	...
	int jj = writeSolarToServer(solar); //empty function
	free(solar);
	pthread_exit(NULL);
}
...
//	victron = getData(3,38);//empty function
//
...
//	//victron = (uint16_t*)malloc(35*sizeof(uint16_t));
//	victron = getData(259,304); //empty function
...
//	victron = getData(778,785);//empty function
...
//		/*repeat for third chunk of data*/
//	//victron = (uint16_t*)malloc(8*sizeof(uint16_t));
//	victron = getData(2600,2608);//empty function
